...
# Upload scripts
aws s3 cp ../pipeline/run_advntr.sh s3://ssc-gangstr-denovos/scripts/ || echo "run_gangstr_chrx.sh upload to s3 failed"
aws s3 cp ../src/decrypt.py s3://ssc-gangstr-denovos/scripts/ || echo "decrypt.py upload to s3 failed"
aws s3 cp ../files/encrypt_code.txt s3://ssc-gangstr-denovos/scripts/encrypt_code.txt ||  echo "encrypt_code.txt upload to s3 failed"
aws s3 cp ../files/ssc_crams_sample_data.csv s3://ssc-gangstr-denovos/scripts/ssc_crams_sample_data.csv ||  echo "ssc_crams_sample_data.csv upload to s3 failed"
...
while read -r FAMID CRAMSLIST; do
  echo "Run GangSTR for ${FAMID} chrX"
  aws batch submit-job \
      --job-name ssc-${FAMID} \
      --job-queue ssc-denovo \
      --job-definition ssc-denovos:6 \
      --container-overrides 'command=["run_gangstr_chrx.sh",'"${CRAMSLIST}"','"${FAMID}"','"${ENC_SSC_ACCESS_KEY}"','"${ENC_SSC_SECRET_ACCESS_KEY}"', '"${SSC_PARAMS}"'],environment=[{name="BATCH_FILE_TYPE",value="script"},{name="BATCH_FILE_S3_URL",value="s3://ssc-gangstr-denovos/scripts/run_gangstr_chrx.sh"}]'
