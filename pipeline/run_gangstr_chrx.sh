...
SSC_PARAMS=${5}
echo "[run_gangstr.sh]: ${FAMID}: ${CRAMSLIST}: ${SSC_PARAMS}"
...
# Scripts
aws s3 cp s3://ssc-gangstr-denovos/scripts/decrypt.py ${DATADIR}/${FAMID}/  || die "Error copying decrypt.py"
aws s3 cp s3://ssc-gangstr-denovos/scripts/encrypt_code.txt ${DATADIR}/${FAMID}/  || die "Error copying encrypt_code.txt"
aws s3 cp s3://ssc-gangstr-denovos/scripts/ssc_crams_sample_data.csv ${DATADIR}/${FAMID}/  || die "Error copying ssc_crams_sample_data.csv"
