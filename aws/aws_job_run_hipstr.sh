...
source ../${SSC_PARAMS}
aws s3 cp ../ssc_shared_params.sh s3://ssc-hipstr-spark/scripts/ || echo "ssc_shared_params.sh upload to s3 failed"
aws s3 cp ../${SSC_PARAMS} s3://ssc-hipstr-spark/scripts/ || echo "${SSC_PARAMS} upload to s3 failed"

# Upload scripts
aws s3 cp ../pipeline/run_hipstr.sh s3://ssc-hipstr-spark/scripts/ || echo "run_gangstr_chrx.sh upload to s3 failed"
aws s3 cp ../src/decrypt.py s3://ssc-hipstr-spark/scripts/ || echo "decrypt.py upload to s3 failed"
aws s3 cp ../files/encrypt_code.txt s3://ssc-hipstr-spark/scripts/encrypt_code.txt ||  echo "encrypt_code.txt upload to s3 failed"
aws s3 cp ../files/ssc_crams_sample_data.csv s3://ssc-hipstr-spark/scripts/ssc_crams_sample_data.csv ||  echo "ssc_crams_sample_data.csv upload to s3 failed"
